...
bool trajGeneration();
MatrixXd add_one_node(MatrixXd path, int index);
...
void execCallback(const ros::TimerEvent &e) {
  ...
  case REPLAN_TRAJ: {
    ...
    cout<<"[REPLAN_TRAJ]"<<" start_pt"<<start_pt.transpose()<<endl;
    cout<<"[REPLAN_TRAJ]"<<" start_vel"<<start_vel.transpose()<<endl;
    bool success = trajGeneration();
    if (success)
      changeState(EXEC_TRAJ, "STATE");
    else
...
bool trajGeneration() {
  ...
  cout<<"[trajGeneration]"<<" start_pt"<<start_pt.transpose()<<endl;
  _astar_path_finder->AstarGraphSearch(start_pt, target_pt);
  auto grid_path = _astar_path_finder->getPath();

  // Reset map for next call
  grid_path = _astar_path_finder->pathSimplify(grid_path, _path_resolution,0,(int)grid_path.size()-1);
  ...
  cout<<"[trajGeneration]"<<" start_pt"<<start_pt.transpose()<<endl;
  cout<<"[trajGeneration]"<<" path"<<path.transpose()<<endl;
...
void trajOptimization(Eigen::MatrixXd path) {
  ...
  ROS_INFO("[safecheck] check done");
  MatrixXd repath = path;
  int count = 0;
  while (unsafe_segment != -1) {
    count++;
    if (count>20)break;   
    ROS_INFO("[replan] replan begin");
    repath = add_one_node(repath,unsafe_segment);
...
MatrixXd add_one_node(MatrixXd path, int index){
  ...
  int n = path.rows();
  MatrixXd newpath(n+1,path.cols());
  MatrixXd bottom = path.block(index+1,0,n-index-1,3);
  MatrixXd top = path.block(0,0,index+1,3);
  Vector3d p1 = path.row(index);
  Vector3d p2 = path.row(index+1);
  Vector3d pmid = (p1+p2)/2;
  newpath.block(0,0,index+1,3) = top;
  newpath.row(index+1) = pmid;
  newpath.block(index+2,0,n-index-1,3) = bottom;
  return newpath;
}
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  for(int i = 0; i<Path.rows() - 1; i++){
    Vector3d p1 = Path.row(i);
    Vector3d p2 = Path.row(i+1);
    double d = (p1-p2).norm();
    if(d>pow(v,2)/a)time(i) = (d/v+v/a);
    else time(i) = 2*sqrt(d/a);
  }
  return time;
}
