...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2) {
  ...
  double h;
  Eigen::Vector3d dv = node1->coord-node2->coord;
    // Eigen::Vector3i v3 = start
    // tie breaker 
  double dx = abs(dv(0));
  double dy = abs(dv(1));
  double dz = abs(dv(2));
  double coeff  = dx+0.5*dy+0.1*dz;
  h = dv.norm()+coeff*0.01;
  return h;
}
...
void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  while (!openSet.empty()) {
    ...
    currentPtr->id = -1;
    // ROS_INFO("[AstarGraphSearch] a node is opened");
    ...
    for(int i = 0; i < (int)neighborPtrSets.size(); i++){
      ...
      if(neighborPtr->id == 0 && !isOccupied(neighborPtr->index)){
        neighborPtr-> cameFrom = currentPtr;
        neighborPtr->gScore = currentPtr->gScore + edgeCostSets[i];
        neighborPtr->fScore = getHeu(neighborPtr,endPtr) + currentPtr->gScore;
        neighborPtr->id = 1;
...
vector<Vector3d> AstarPathFinder::getPath() {
  ...
  GridNodePtr c_ptr = terminatePtr;
  while (c_ptr->cameFrom != nullptr){
      gridPath.push_back(c_ptr);
      c_ptr = c_ptr->cameFrom;
    }
...
vector<Vector3d> AstarPathFinder:: pathSimplify(const vector<Vector3d> &path,
                                               double path_resolution, int ind1,int ind2) {
  ...
  {
    double d = perpendicularDistance(path[i],path[ind1],path[ind2-1]);
    if (d > dmax){
      index = i;
      dmax = d;
    }
  ...
  if(dmax > path_resolution){
    ...
    vector<Vector3d> refinedpath1 = AstarPathFinder::pathSimplify(path,path_resolution,ind1,index);
    vector<Vector3d> refinedpath2 = AstarPathFinder::pathSimplify(path,path_resolution,index,ind2);
    for(int k = 0;k<refinedpath1.size()-1;k++){
      subPath.push_back(refinedpath1[k]);
    }
    for(int k = 0;k<refinedpath2.size();k++){
      subPath.push_back(refinedpath2[k]);
    }
  }
  else{
    subPath.push_back(path[ind1]);
    subPath.push_back(path[ind2]);
  }

  return subPath;
}

inline double AstarPathFinder::perpendicularDistance(const Eigen::Vector3d aimpoint,const Eigen::Vector3d point1,const Eigen::Vector3d point2) const{
  return ((aimpoint-point1).cross(aimpoint-point2)).norm()/((point2-point1).norm());
}
...
int AstarPathFinder::safeCheck(MatrixXd polyCoeff, VectorXd time) {
  ...
  while (cnt<m){
    ...
    Vector3d p = getPosPoly(polyCoeff,cnt,t);
